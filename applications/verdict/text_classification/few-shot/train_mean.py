        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (current_batch_id != prev_batch_id).any() or step == len(epoch_iterator) - 1 or len(epoch_iterator) == 1:
                    ...
                    accum_logits = accum_logits / count
                    ...
                    # print(logits)
                    accum_logits = paddle.add(accum_logits, logits.sum(axis=0, keepdim=True) / self.args.train_batch_size)
            ...
            if (current_batch_id != prev_batch_id).any() or step == len(dataloader) - 1 or len(dataloader) == 1:
                ...
                with paddle.no_grad():
                    ...
                    if has_labels:
                        with self.autocast_smart_context_manager():
                            _, logits = self.compute_forward(model, inputs)
                            accum_logits = paddle.add(accum_logits, logits.sum(axis=0, keepdim=True) / self.args.eval_batch_size)
                            continue
                count = 1 if count == 1 else count + 1
